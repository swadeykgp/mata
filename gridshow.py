...
class Animation:
  def __init__(self, map, schedule):
    ...
    for o in map["map"]["obstacles"]:
      ...
      self.patches.append(Rectangle((x - 0.5, y - 0.5), 0.5, 0.5, facecolor='black', edgecolor='black'))
    ...
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      self.patches.append(Rectangle((d["pick"][0] - 0.25, d["pick"][1] - 0.25), 0.5, 0.5, facecolor='yellow', edgecolor='red', alpha=0.5))
    
    # draw drop first
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      self.patches.append(Rectangle((d["drop"][0] - 0.25, d["drop"][1] - 0.25), 0.5, 0.5, facecolor='blue', edgecolor='red', alpha=0.5))
    
    # draw goals first
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      self.patches.append(Rectangle((d["goal"][0] - 0.25, d["goal"][1] - 0.25), 0.5, 0.5, facecolor='green', edgecolor='black', alpha=0.5))
...
if __name__ == "__main__":
  ...
  with open('/home/swarnava/Projects/Work/aiml/aifa/assignment/conflict_resolution/input_ass1.yaml') as map_file:
    map = yaml.load(map_file, Loader=yaml.FullLoader)

  with open('/home/swarnava/Projects/Work/aiml/aifa/assignment/conflict_resolution/output_ass1.yaml') as states_file:
